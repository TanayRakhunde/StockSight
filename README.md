...
StockSight was developed to demonstrate end-to-end data engineering, machine learning, and frontend development capabilities. The application seamlessly integrates financial data APIs, custom web scraping pipelines, and a predictive machine learning model into a unified, responsive user interface. 

It serves as a comprehensive dashboard for equity research, allowing users to aggregate historical data, view predictive trends, and process headlines from multiple sources simultaneously.

## Architecture & Features

### Frontend Architecture (User Interface & Visualization)
...
- **Dynamic Metric Cards:** Real-time calculation and display of model performance metrics (R-squared Score) and dynamic financial indicators (Market Cap, 52-Week High/Low).
- **Asynchronous Execution:** Utilizes Streamlit's session state and caching to provide a Single Page Application (SPA) experience without full page reloads.

### Backend Architecture (Data Pipelines & Aggregation)
...
- **Multi-Source News Scraping:** Employs BeautifulSoup4 and Requests to scrape HTML tables from public financial platforms while respecting server load with programmatic delays and standard User-Agents.
- **RSS Feed Parsing:** Implements feedparser to fetch, parse, and normalize live XML RSS feeds from Google News and Yahoo Finance.
...
- **Caching Layer:** Heavy network calls and data manipulations are decorated with cache directives using Time-To-Live (TTL) expiration, drastically reducing API rate limits and improving application latency.
